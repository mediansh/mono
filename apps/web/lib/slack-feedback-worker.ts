...
export async function processSlackFeedbackInBackground(args: { integrationId: Id<"slackWorkspaceIntegrations"> }) {
  ...
  try {
    ...
    logger.info("[debug] Slack classifier raw text", { integrationId: args.integrationId, text: classifierResult.text, pendingCount: pendingMessages.length, transcript })
    const classification = feedbackClassificationSchema.parse(JSON.parse(extractJsonObject(classifierResult.text)))
    logger.info("[debug] Slack classifier parsed", { integrationId: args.integrationId, classification })
    ...
    logger.info("[debug] Slack extractor result", { integrationId: args.integrationId, hasOutput: Boolean(extractorResult.output), output: extractorResult.output, text: extractorResult.text })
    if (!extractorResult.output) {
      logger.warn("Slack feedback extractor produced no structured output", { integrationId: args.integrationId })
      await client.slack.markFeedbackWindowProcessed(botSecret, args.integrationId, latestPendingMessage.messageTs, latestPendingMessage.messageCreatedAt)
      await trackFeedbackProcessing({ distinctId: feedbackWindow.integration.workspaceId, platform: "slack", integrationId: args.integrationId, workspaceId: feedbackWindow.integration.workspaceId, messageCount: pendingMessages.length, isProductFeedback: classification.isProductFeedback, confidence: classification.confidence, createdTaskCount: 0, updatedTaskCount: 0, classifierDurationMs, extractorDurationMs, totalDurationMs: Date.now() - processingStart })
      await client.slack.finalize(botSecret, args.integrationId, { kind: "success", reason: "no_structured_output" })
      return
    }
    ...
    const operations: SlackFeedbackTaskOperation[] = limitedActions.map((action) => action.action === "create" ? ({ action: "create", task: { title: action.title, description: action.description ?? undefined, status: "requests", priority: action.priority ?? "none", labels: action.labels.filter((label) => feedbackWindow.integration.availableLabels.includes(label)).slice(0, MAX_EXTRACTED_LABELS), source: relevantMessages[0]?.permalink ? { platform: "slack", url: relevantMessages[0].permalink, author: relevantMessages[0].authorUsername } : undefined, createdAtLabel: formatCreatedAtLabel(relevantMessages[0]?.messageCreatedAt ?? latestPendingMessage.messageCreatedAt) } }) : ({ action: "update", taskCode: action.taskCode, title: action.title, description: action.description ?? undefined, priority: action.priority ?? undefined, labels: action.labels.filter((label) => feedbackWindow.integration.availableLabels.includes(label)).slice(0, MAX_EXTRACTED_LABELS) }))
