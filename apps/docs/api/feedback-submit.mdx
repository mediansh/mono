...
A minimal feedback widget can post directly to the API. Keep the key on a server you control and proxy the request through it whenever possible.

```html
<script>
  async function sendFeedback(content) {
    await fetch("https://api.median.sh/api/feedback", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        Authorization: "Bearer " + window.MEDIAN_KEY,
      },
      body: JSON.stringify({
        content,
